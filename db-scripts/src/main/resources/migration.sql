...
UPDATE `genetic_profile` SET `NORMALS_TISSUE_REFERENCE_ID`='hgu133plus2' 
	WHERE `GENETIC_ALTERATION_TYPE`='MRNA_EXPRESSION'
	AND	`DATATYPE`='CONTINUOUS'
    AND `STABLE_ID` LIKE '%_mrna_U133';

UPDATE `genetic_profile` SET `NORMALS_TISSUE_REFERENCE_ID`='gtex'
	WHERE `GENETIC_ALTERATION_TYPE`='MRNA_EXPRESSION'
	AND	`DATATYPE`='CONTINUOUS'
    AND (`STABLE_ID` LIKE '%rna_seq_mrna'
    OR `STABLE_ID` LIKE '%rna_seq_v2_mrna'
    OR `STABLE_ID` LIKE '%rna_seq_mrna_capture');
...
##version: 2.7.4
ALTER TABLE `cancer_study` CHANGE `IS_ADULT_CANCER` `IS_PEDIATRIC_CANCER` BOOLEAN DEFAULT FALSE;
UPDATE `cancer_study` SET `IS_PEDIATRIC_CANCER` = !`IS_PEDIATRIC_CANCER`;
ALTER TABLE `genetic_profile` DROP COLUMN `NORMALS_TISSUE_REFERENCE_ID`
